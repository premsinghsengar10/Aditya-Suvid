...
- **Hero Banner Carousel**: Replaced the single hero media with a two-slide carousel that opens on the banner video, switches to the banner image with side arrows, and can return to the video with autoplay and video controls preserved.
- **Harmoniq Creative Brand Asset**: Centralized the new assets/brands/harmoniq-creatives.webp image and replaced legacy Harmoniq Creative visuals across Entities, Companies, and Harmoniq Creative article cards.
...
- **Form Abuse Protection**: Added a three-day normalized-email lock and deterministic SHA-256 `idempotency_key` to the Contact and Career forms. This is a client-side safeguard; enforceable cross-device rate limiting and idempotency still require a server/edge layer.
